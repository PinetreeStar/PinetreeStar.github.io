    ...
    xhttp.onload = function () {
        ...
        if (fullArr[0] == 'TRUE'){
            ...
            if (inject){
                document.getElementById("chartSection").innerHTML = "";
                document.getElementById("mapSection").innerHTML = "";
                if (infos.includes('cart_latitude')){
                    document.querySelector("body").style.gridTemplateColumns = "60% 40%";
                    document.getElementById("mapSection").insertAdjacentHTML('beforeend', 
                    `<div id="map_holder"><p>Map data: ${carts.join(', ')}</p><div id="map"></div></div>`);
                }else{
                    document.querySelector("body").style.gridTemplateColumns = "100%";
                }
            }
            ...
            for (let c = 0; c < carts.length; c ++){
                ...
                if ((latData != null) && (lngData != null) && (altData != null)){
                    dict.set('timestamp', stamps);
                    addCartData(carts[c], dict);
                }
            }
        }else{
...
//Pass cart, then a dictionary with all labels as keys and data as values
function addCartData (cart, dict){
    ...
    for (let i = 0; i < dict.get("cart_latitude").length; i++){
        
        var cartLocation =  {
            lat: parseFloat(dict.get("cart_latitude")[i]), 
            lng: parseFloat(dict.get("cart_longitude")[i])
        };
        //console.log("lat and lng: ", cartLocation.lat, cartLocation.lng);
        //var elevation = data[2][i];
        data = mapToString(dict, cart, i);
        // `
...
function mapToString(dict, cart, i){
    ret = "Cart: " + cart.split('_').map(capitalizeWord).join(' ') + "<br>";
    dict.forEach(function(value,key){
        ret += key.split('_').map(capitalizeWord).join(' ') + ": " + value[i] + "<br>";
    });
    return ret;
}